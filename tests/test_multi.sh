...
sleep 4
assert "work 視窗數" "1" "$(pgrep -f "scrcpy .*$P1:5555" | grep -c .)"
assert "test 視窗數" "1" "$(pgrep -f "scrcpy .*$P2:5555" | grep -c .)"
"$PM" reset -p work >/dev/null 2>&1
sleep 1
assert "reset work 後 work 視窗被關" "0" "$(pgrep -f "scrcpy .*$P1:5555" | grep -c .)"
...
assert "實際跑著兩個 scrcpy" "2" "$(pgrep -f 'scrcpy .*100.101.102.1' | grep -c .)"
pkill -f 'scrcpy .*100.101.102.1' 2>/dev/null; sleep 1
