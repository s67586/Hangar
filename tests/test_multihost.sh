...
prep
out="$("$PM" setup --transport tailscale --existing pixel < /dev/null 2>&1)"; rc=$?
assert "離開碼 0"              "0" "$rc"
...
prep
out="$("$PM" setup --transport tailscale pixel < /dev/null 2>&1)"
...
out="$("$PM" setup --transport tailscale --existing pixel < /dev/null 2>&1)"
...
prep
"$PM" setup --transport tailscale --existing pixel --name mac2 < /dev/null >/dev/null 2>&1
