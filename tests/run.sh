...
# 也不會真的對區網送封包。hub 與 helper 的測試會在 127.0.0.1 上開一個隨機埠。
#
#   tests/run.sh
#
set -uo pipefail
...
for suite in test_core.sh test_multi.sh test_adb_race.sh test_multihost.sh test_json.sh test_scan.sh test_hub.sh test_helper.sh test_agent_protocol.sh test_agent_client.sh test_versions.sh test_manual.sh; do
  echo
  echo "############ $suite ############"
