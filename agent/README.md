...
而那本來就需要 adb，也就是需要已經有人實體碰過這支手機。手機已經入伍、但這台
電腦手上沒有 token 的時候（profile 重建過，或它是別台電腦入伍的），不必自己打這
一行 —— `hangar enroll -p <手機> --takeover` 就是「清掉再重新入伍一次」，它會先
把代價講清楚並要你當面點頭。token 存在 app 的私有資料裡，adb 這一側讀不回來，
所以沒有第三條路。
