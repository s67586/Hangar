...
| 網段互相隔離時的跨網段 | **遠期目標**。網段之間兩個方向都不通時，只剩走網際網路；目前的答案是 Tailscale（見「[遠期：網段完全隔離時怎麼跨](#遠期網段完全隔離時怎麼跨)」） |
...
**不在目前的路線上。** 起因是實際碰到的網路：同一個公司 Wi-Fi（同一個 SSID）
把裝置分到不同的 /24，同一段裡互通，**跨段兩個方向都不通**（連對方那段的
閘道都 ping 不到），但每一段都出得去網際網路；IT 不開放跨段、也不固定分段。

現有的兩條跨網段路（agent 主動回報 `--checkin`、`scan --subnet` 逐台探埠，見
「[誰問誰](#誰問誰預設由電腦端拉跨網段時可選-agent-主動推)」）都需要**至少一個方向通**，
在這種網路上用不上。兩邊唯一共同連得到的是網際網路，所以剩下的方案都經過它：

| 方案 | 看得到狀態 | 響鈴／切偵錯 | 投影 | 代價 |
|---|---|---|---|---|
| **T. 測試機裝 Tailscale** | ✓ | ✓ | ✓ | 不用改程式；每支手機要裝、要登入。UDP 打洞被擋時走 DERP 中繼，hangar 會自動降畫質（實測：Pixel 8a 經香港 DERP 約 60 ms，投影可用） |
| **R1. 回報端點公開到網際網路**（例如 Cloudflare Tunnel），agent 用 `--hub https://…` | ✓ | ✗ | ✗ | 幾乎不用改程式：`--checkin` 與 https 的 hub 網址都已支援。要處理的是「把 hub 的一個埠公開出去」的維運與安全 |
| **R2. R1 ＋ 回報時領取指令** | ✓ | ✓ | ✗ | 新功能：hub 排指令，agent 回報時順便領走、執行、下次回報帶回結果。指令最慢一個回報週期才生效 |
| **R3. R2 ＋ agent 開常駐通道轉 adb** | ✓ | ✓ | ✓ | 等於自己重做一次 Tailscale，見下面 |

**目前的建議**：要投影就用 T；不想每支都裝 Tailscale、只要看狀態與響鈴／切偵錯，
做 R2。R3 不建議。

**為什麼 R1／R2 投不了影**：scrcpy 靠一條 Mac 的 adb 連到手機 5555 的 TCP 連線，
整段投影都要開著，畫面串流（每秒數 Mbps）與觸控操作都在上面、兩個方向都要即時。
R1／R2 只有「手機每隔一段時間送一個小 HTTP 請求」，hub 無法主動連進手機 ——
響鈴、切偵錯這種晚幾十秒也無妨的小指令塞得進去，投影塞不進去。

**R3 要解決的事**（做出來就是一個小型 Tailscale）：

```
Mac 的 adb ⇄ 網際網路上的中繼 ⇄ agent 主動開的常駐通道 ⇄ 手機本機的 adbd（5555）
```

| 要解決的 | 為什麼麻煩 |
|---|---|
| 中繼伺服器 | 要自己架、自己維運；投影流量全部經過它（每支每秒數 Mbps），有頻寬成本 |
| 安全 | 等於把 adb 經網際網路開出去，而 adb 幾乎能對手機做任何事，驗證要極嚴 |
| 5555 會消失 | 重開機後就沒了，agent 開不回來（只有 shell／root 設得動，見 M3c），要有人再用 USB 開 |
| 延遲 | 走中繼，跟 Tailscale 走 DERP 同級甚至更差 |

另一條不經過 adb 的路是把上一節的「網頁投影串流」（agent 用 MediaProjection）
接到 R2 的中繼上：畫面經中繼送進瀏覽器。它同樣卡在 D1／D2，而且一樣要中繼扛
影像頻寬，所以跟上一節一起看，不單獨排。

**什麼時候重新評估**：有測試機無法裝 Tailscale（例如公司管理的裝置不准裝 VPN），
或者要同時管的手機多到逐支登入 Tailscale 變成負擔 —— 那時先做 R2。
