...
把裝置分到不同的 /24，同一段裡互通，**跨段兩個方向都不通**，但每一段都出得去
網際網路。現有的 `--checkin` 與 `scan --subnet` 都需要至少一個方向通，用不上；
兩邊唯一共同連得到的是網際網路。

| 方案 | 看得到狀態 | 響鈴／切偵錯 | 投影 | 代價 |
|---|---|---|---|---|
| **R1. 回報端點公開到網際網路**（例如 Cloudflare Tunnel），agent 用 `--hub https://…` | ✓ | ✗ | ✗ | 幾乎不用改程式：`--checkin` 與 https 的 hub 網址都已支援 |
| **R2. R1 ＋ 回報時順便領取指令** | ✓ | ✓ | ✗ | 新功能：hub 排指令，agent 回報時領走、執行，下次回報帶回結果 |
...
不會產生任何自動狀態變更。**已完成**：hub 記住每支手機最後一次看到的偵錯開關，
agent 叫不動而最後一次是關的，卡片標 `stranded`、紅框橫幅、排最前面（見
[docs/hub.md](docs/hub.md)「那頁上的狀態是什麼意思」）。還差的一塊是那份記憶
只放在記憶體，hub 重開就忘了。
...
A1–A3、B1、B2 有照表跑的腳本：`tools/field_check.sh`（`watch`／`reboot`／`adb-off`／
`wifi-adb`／`repair`），會動手機的那幾項執行前會先問、跑完改回原狀。
